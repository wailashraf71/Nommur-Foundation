...
const app = new Vue({
    ...
    methods: {
        ...
        deleteRowAdvert(data) {
            var csrf = $('meta[name="csrf-token"]').attr('content');
            $('#format').html('<form id="dynForm" action="http://45.77.143.238/admin/adverts/' + data.id + '" method="post"><input type="hidden" name="_method" value="DELETE"><input type="hidden" name="_token" value="' + csrf + '"></form>')
        },
        deleteRowUser(data) {
            var path = window.location.href;
            var csrf = $('meta[name="csrf-token"]').attr('content');
            $('#format').html('<form id="dynForm" action="http://45.77.143.238/admin/users/' + data.id + '" method="post"><input type="hidden" name="_method" value="DELETE"><input type="hidden" name="_token" value="' + csrf + '"></form>')
        },
    },
});
